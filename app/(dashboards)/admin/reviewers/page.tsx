...
import { getReviewersPaginated } from "@/lib/mock/admin";
import { cn } from "@/lib/utils";

export default function ReviewersListPage() {
  const [page, setPage] = useState(1);
  const { data, totalPages } = getReviewersPaginated(page, 10);

  return (
    <div className="flex flex-col gap-6">
      <SectionHeader title="Reviewers List" subtitle="English" />
                ...
                >
                  <td className="w-12 px-4 py-3 text-center">
                    <input
                      type="checkbox"
                      className="h-4 w-4 cursor-pointer rounded border-border accent-primary"
                      aria-label={`Select reviewer ${reviewer.id}`}
                    />
                  </td>
                  <td className="px-4 py-3">{reviewer.name}</td>
                  <td className="px-4 py-3">{reviewer.email}</td>
                  <td className="px-4 py-3">{reviewer.dateJoined}</td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      aria-label={`Delete reviewer ${reviewer.id}`}
                    >
